...
namespace RoxEngine::log {
	...
	template<typename T>
	inline void trace(const T& msg) { sLogger.trace(std::format("{}",msg)); }
	template<typename T>
	inline void info(const T& msg)  { sLogger.info(std::format("{}",msg)); }
	template<typename T>
	inline void warn(const T& msg)  { sLogger.warn(std::format("{}",msg)); }
	template<typename T>
	inline void error(const T& msg) { sLogger.error(std::format("{}", msg)); }
	template<typename T>
